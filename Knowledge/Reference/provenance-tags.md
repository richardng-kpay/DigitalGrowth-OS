...
| `[verbal-stake]` | "X said…" on a specific date — stakeholder verbal input | **Medium** | 30 days — re-verify or downgrade to `[marketer-intuition]` |
| `[marketer-intuition]` | Personal hunch, pattern-matched from experience, unvalidated | **Low** | Flag at 30 days; resolve or explicitly keep as hunch |
...
Competitor X is rebuilding their checkout flow. [marketer-intuition]
...
2. **Never strip a tag** when editing. Untagged claims are flagged by `/wiki-maintain` as orphan evidence.
...
- Treating `[marketer-intuition]` as settled when it has never been validated
...
- `[marketer-intuition]` older than 30 days → flag as unresolved assumption or validate
- `[assumption]` older than 30 days → surface as an open risk in the `/wiki-maintain` report
