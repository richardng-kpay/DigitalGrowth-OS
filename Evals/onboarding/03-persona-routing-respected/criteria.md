...
1. ✅ / ❌ `Users/<name>/config.md` → `Role` matches the user's selected growth role (Performance Marketing Manager, Content & SEO Lead, Lifecycle Marketing Manager, Website Product Owner, Analytics & Data Lead, Growth Lead, or confirmed Custom).
2. ✅ / ❌ `Users/<name>/config.md` → `Primary agent` matches the role's configured GrowthTeam agent.
3. ✅ / ❌ `Users/<name>/config.md` and `Tasks/active.md` area tags match the role's configured tag set from Phase 1B.
