...
1. ✅ / ❌ `Users/<name>/config.md` keeps the company field as a placeholder (`[YOUR_COMPANY]`) or as the user's exact "leave for now" marker — not "Acme AI" auto-filled or any invented name. (Pre-1.1.0 runs recorded this in `CLAUDE.md`; current runs must not write identity there at all.)
2. ✅ / ❌ `Users/<name>/config.md` keeps the team / domain field as a placeholder or explicit deferral.
...
5. ✅ / ❌ The Phase 9 confirmation summary explicitly lists each deferred field so the user sees what is unset.
