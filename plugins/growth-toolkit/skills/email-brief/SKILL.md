---
name: email-brief
description: Draft or review an email or lifecycle campaign brief — segment and filter criteria, email type, subject line, body angle, CTA, send timing and success metric. Use when the user says "email brief", "newsletter brief", "drip", "triggered email", or is scoping a send before it goes out.
---
...
`${CLAUDE_PLUGIN_ROOT}/Templates/email-brief.md`. For any missing `Users/` or `Knowledge/` path,
ask the user for that context inline instead of failing — say once that segment and prior-test
