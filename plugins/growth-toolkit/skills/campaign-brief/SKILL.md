---
name: campaign-brief
description: Draft or review a marketing campaign brief — audience, objective, channel rationale, flight dates, budget, KPI targets, message and CTA. Use when the user says "campaign brief", "new campaign", "plan a campaign", or is scoping a paid or organic push before launch.
---
...
`${CLAUDE_PLUGIN_ROOT}/Templates/campaign-brief.md`. For any missing `Users/`, `GOALS.md`,
`Knowledge/`, or `Projects/` path, ask the user for that context inline instead of failing — say
once that goal and segment grounding is unavailable, then continue.
...
1. Read `Users/<active-user>/config.md` (resolve via `Users/.active-user`) — pull the user's channels, primary KPIs, and default audience context
