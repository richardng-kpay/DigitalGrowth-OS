...
description: Turn a bounded research corpus into a grounded synthesis — themes, pain points, conflicting signals, implications and open questions, with every claim traced to a source. Use for interview notes, customer-call excerpts, survey free text, and discovery notes, or when the user says "synthesize this research" or "what are the themes".
---
...
Installed from the `growth-toolkit` plugin rather than a Digital Growth OS clone, there is no
`Knowledge/Ingestion/` to file findings into — offer the synthesis as a file the user chooses a
home for instead. The eval suite referenced at the end lives at
`${CLAUDE_PLUGIN_ROOT}/Evals/research-synthesis/`.
