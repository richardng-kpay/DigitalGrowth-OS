---
name: brief-review
description: Pre-publish quality gate for any external marketing artifact — checks audience fit, message clarity, CTA strength and tone via the Creative Reviewer. Use before a campaign brief is approved, an email or ad goes live, or content or landing page copy is published; also when the user says "review this before it ships".
---
...
paths below will not exist. For any missing `Users/` or `Knowledge/` path, ask the user for that
context inline instead of failing — say once that segment and brand-voice grounding is
unavailable, then continue. The Creative Reviewer agent ships with the plugin, so the review
itself always runs.
