...
description: Design or validate an A/B test or growth experiment — hypothesis, variants, primary metric, baseline, minimum detectable effect, sample size and read date. Use when the user says "experiment brief", "A/B test", "run a test", "is this test valid", or is proposing a change they want to prove.
---
...
`${CLAUDE_PLUGIN_ROOT}/Templates/experiment-brief.md`. For any missing `Users/` or `Knowledge/`
path, ask the user for that context inline instead of failing — say once that prior-experiment
grounding is unavailable, then continue.
