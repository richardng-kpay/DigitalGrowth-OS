---
name: content-brief
description: Draft or review a content or SEO brief — topic, primary reader, funnel stage, target keyword, search intent, competitor URLs, CTA and length. Use when the user says "content brief", "SEO brief", "blog post brief", or is scoping a piece before writing starts.
---
...
`${CLAUDE_PLUGIN_ROOT}/Templates/content-brief.md`. For any missing `Users/` or `Knowledge/` path,
ask the user for that context inline instead of failing — say once that segment and competitor
grounding is unavailable, then continue. The Lark wiki search in step 2 applies only inside the
full OS; skip it when the `lark-mcp` server is not connected.
