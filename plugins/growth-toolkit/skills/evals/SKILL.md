---
name: evals
description: Run or prepare an eval suite with transcript capture, model and commit pinning, and independent grading. Use when the user says "run the evals", "test this skill", "did this regress", or wants to measure whether an AI workflow actually works before trusting it.
---
...
Installed from the `growth-toolkit` plugin rather than a Digital Growth OS clone, the suites live
at `${CLAUDE_PLUGIN_ROOT}/Evals/` and that directory is read-only. Write run results to a path the
user chooses in their own project, not back into the plugin.
