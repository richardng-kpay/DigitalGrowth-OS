...
description: Pre-publish review of any external-facing artifact — campaign briefs, ad copy, subject lines, landing page and content drafts — for audience fit, message clarity, CTA strength and brand voice. Use before anything ships to customers, or when the user asks for an honest read on copy.
...
**Outside the full OS:** running from the `growth-toolkit` plugin, templates are at `${CLAUDE_PLUGIN_ROOT}/Templates/` and the rest of these workspace files do not exist. Ask the user for the audience and brand-voice context directly rather than reporting a missing file — and review against what they actually told you, not an assumed house style.
...
| Full campaign brief review, landing page review | `claude-sonnet-5` — standard |
| Brand voice audit, full lifecycle review | `claude-opus-5` — depth |
