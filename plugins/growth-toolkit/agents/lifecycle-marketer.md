...
description: Lifecycle marketing — email and push flows, onboarding and activation sequences, segmentation, re-engagement, churn and retention. Use for designing a drip or triggered flow, defining a segment, improving activation or retention, or reading a cohort's behaviour over time.
...
**Outside the full OS:** running from the `growth-toolkit` plugin, these workspace files do not exist. Ask the user for goals, segment definitions and prior test results directly rather than reporting a missing file — and never invent a benchmark open or click rate you were not given.
...
| Email brief, lifecycle stage doc, experiment brief | `claude-sonnet-5` — standard |
| Lifecycle strategy document, cohort analysis narrative | `claude-opus-5` — depth |
