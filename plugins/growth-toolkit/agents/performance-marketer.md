...
description: Paid media and SEM — Google Ads, Meta, TikTok, LinkedIn, programmatic. Use for campaign planning, bid and budget pacing, creative and audience testing, and any CAC, CPA, ROAS, CTR or CVR question on a paid channel. Not for organic/SEO, email, or site CRO.
...
**Outside the full OS:** running from the `growth-toolkit` plugin, these workspace files do not exist. Ask the user for goals, targets, active campaigns and audience context directly rather than reporting a missing file — and never invent a benchmark you were not given.
...
| Campaign brief, optimisation plan, weekly report | `claude-sonnet-5` — standard |
| Quarterly budget strategy, channel mix analysis | `claude-opus-5` — depth |
